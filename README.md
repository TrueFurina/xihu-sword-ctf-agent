...
| LLM reasoning on the held-out pool (measured 2026-09-19) | pool **1 / 10** solved — the sole solve is **deterministic presolve** (`real_reverse_js`), **not LLM**; **LLM autonomous reasoning 0 / 10** |

> **Why we never divide 14 by 10.** The 14 KPI solves and the 10 held-out problems are **disjoint sets**. `14 / 10 = 140%` would be a *deceptive ratio*, so our tooling hard-codes `coverage_of_heldout=None` instead of emitting a number. Under the held-out yardstick (measured 2026-09-19; raw reports archived in `ctf_agent/heldout_evidence/`), **LLM autonomous reasoning 0 / 10**. Doubling the token budget (`566,570 → 696,506`) drove `budget_exceeded` (`6 → 0`) but left LLM solves at 0: **the bottleneck is capability, not budget**.
...
We publish this repository while stating its numbers plainly, including the ones that are unflattering. The strict KPI is **14 solved with full evidence** out of **92 collected real problems** (≈**15.2%**), where "solved" means a deterministic, re-runnable solver produces the flag and it matches the problem statement's SHA-256 — the LLM contributes **0 of 14**. On the **held-out pool** (10 unseen, non-trivial problems, disjoint from the 14), the LLM solved **0 of 10** in a real run (566,570 tokens); the pool's single solve came from the deterministic pre-solve layer, not the model. Doubling that budget to 696,506 tokens removed every budget-exhaustion failure but produced **zero** additional solves — the bottleneck is model capability, not spend. `14/10` is never computed. All of the above rests on artifacts in this repository; you can re-run them yourself.
