...
| Metric | Result |
|--------|--------|
| **offline_verified** (strict real-problem KPI) | **11** |
...
| LLM autonomous-reasoning contribution | **0** (all 11 verified solves are deterministic presolve/tooling; zero LLM reasoning) |

> ⚠️ **What `offline_verified=11` does and does NOT mean.** It is the count of *real past-CTF problems* (provenance=`real_past_ctf`, sha256-verified, machine-counted by `scripts/_merge_gate.py count_offline_verified`) solved by a **reproducible deterministic pipeline** — a real engineering milestone, but **NOT a capability measurement**. These problems' writeups are public and almost certainly in LLM pre-training corpora (contamination risk — see `data/results/CTF-Agent深度评审报告_20260828.md` G1). LLM autonomous-reasoning contribution is **0/11**. Treat 11 as "template coverage of memorizable public problems," never as "reasoning ability." (Note: `real_crypto_ezrsa` was promoted back into the strict KPI on 2026-09-03 after a deterministic Håstad broadcast solver — `skills/crypto_hastad_broadcast.py`, wired into presolve as `_try_hastad_broadcast` — returned `REGRESS_PASS` (e=17, sha256 matches ground truth); `real_crypto_simplelegendre` was promoted back the same day after a deterministic Legendre-symbol solver — `skills/crypto_legendre_phi.py`, wired into presolve as `_try_legendre_phi` — returned `REGRESS_PASS` (phi-leak factorization of the 1024-bit modulus + per-bit `(c|p)=(-1)^bi` recovery, sha256 matches ground truth); see `PROMOTION_EVIDENCE` in `scripts/_antifraud.py`. `real_crypto_exciting_inverse` remains **not reproducible** by the deterministic pipeline — removed from the strict KPI on 2026-08-28 after `presolve` returned `None`; see `KNOWN_GAP`.)

> **LLM reasoning break-ice experiment (not counted in KPI)**: With `scripts/_llm_breaking_ice.py --no-internal-presolve` (presolve shortcuts disabled, LLM forced to reason), `8/15` historical problems were solved by `main_agent_llm` autonomous reasoning on baidu Qianfan ERNIE (crypto×6 / reverse×1 / web×1; all sha256-verified — disk truth `deliverables/benchmark_runs/llm_breaking_ice_20260828-022534.json`; an earlier `10/15` count included two reverse solves whose sha256 did not match and was corrected). This proves real LLM reasoning capability exists, but it is **not** counted in `offline_verified` — that KPI only admits strict sha256-verified solves, and the 11 it holds are presolve-deterministic. Details in `MEMORY.md`.
