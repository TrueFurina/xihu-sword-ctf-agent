...
| Regression-set reproducible count | **15 / 15** (12 题集 presolve + 10732/10735/specialcurve2 治理修复：`scripts/verify_10732.py` + `verify_10735.py` + `verify_specialcurve2.py` 可机器复现攻击链；REGRESSION_CHECKS 15 道全过；10732/10735 **不进** PROMOTION_EVIDENCE 因无外部 sha256 真值闭环；specialcurve2 有官方 flag_sha256 闭环经 PROMOTION_EVIDENCE 带证据晋级 12→13，详见台账题块 1/2 + `scripts/_antifraud.py`) |
