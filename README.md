...
> **LLM reasoning break-ice experiment (not counted in KPI)**: With `scripts/_llm_breaking_ice.py --no-internal-presolve` (presolve shortcuts disabled, LLM forced to reason), **`11/15`** historical problems were solved by `main_agent_llm` autonomous reasoning on baidu Qianfan ERNIE (crypto×7 / reverse×3 / web×1; all 11 have `sha256_ok=true` and `validated=true` against the problem ground truth — disk truth `deliverables/benchmark_runs/llm_breaking_ice_20260901-050201.json`). The 4 unsolved are `real_crypto_dnui_keyboard` / `real_crypto_specialcurve2` / `real_misc_vnctf_flag` (all TIMEOUT) and `real_misc_xuanhun_signin` (wrong_direction). Earlier runs: `8/15` (2026-08-28, `llm_breaking_ice_20260828-022534.json`) is **superseded** by the 2026-09-01 run; an even earlier `10/15` count included two reverse solves whose sha256 did not match and was corrected. This proves real LLM reasoning capability exists, but it is **not** counted in `offline_verified` — that KPI only admits strict sha256-verified solves, and the 13 it holds are presolve-deterministic/tooling. Details in `MEMORY.md`.
