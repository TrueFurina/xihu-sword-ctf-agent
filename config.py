...
def _resolve_provider_defaults(provider: str) -> tuple[str, str]:
    ...
    if provider == "qwen":
        ...
        # 默认模型用 deepseek-v4-pro-0813（2026-08-26 实测 HTTP 200 可用，用户账号
        # 该模型 1M tokens 额度 258.5K 已用 74.15% 使用中；深推理模型单题 3-5 分钟，
        # 需更长墙钟(180-240s)——A/B 验证主 Agent 纯 LLM 能力用）——此前默认 qwen3.7-plus
        # 免费额度已耗尽（403 Free quota exhausted），导致 qwen provider 熔断。
