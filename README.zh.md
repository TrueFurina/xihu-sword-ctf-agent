...
> ⚠️ **口径区分（勿混用）**：除上述 **2 题「能力分母」**外，仓库里还有一个 **17 题「可选跑池」**（2 道自有 + 15 道外部 Google CTF 采源），它是**扩池候选，不是能力分母**。该池难度显著更高（实测单题 token 成本约为干净池的 ~50 倍），在 100K tokens/题的护栏下抽样跑过的题**全部未解出**，且其报告被机器标记为 `integrity.interpretable=false` —— 即**不允许被当作能力率引用**。把 `0/17` 或 `1/17` 当成能力率，等于把「题更难」说成「能力更低」，是欺骗性比率。两个口径由 `scripts/_kpi_canonical.py` 分别输出为 `heldout_candidates`（=2，能力分母）与 `heldout_runnable_pool`（=17，可选跑池）。
