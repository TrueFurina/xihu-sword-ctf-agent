"""主 Agent 真打 web-002~web-007（强制 presolve-skip，走 LLM 全链路），验证泛化。

目的：web-001 已证明"LLM 在 web 上的能力缺口本质是工具缺陷（GET vs POST / 缺真执行靶），
不是 LLM 质量问题"。本脚本验证该结论能否**泛化**到更多 web 漏洞类型，把 LLM 贡献从 1 推到 7。

靶机需先起（各自一个终端 / 或后台）：
    .venv/Scripts/python.exe scripts/web_target_ssti.py        # 9002
    .venv/Scripts/python.exe scripts/web_target_unserialize.py  # 9003
    .venv/Scripts/python.exe scripts/web_target_traversal.py   # 9004
    .venv/Scripts/python.exe scripts/web_target_jwt.py         # 9005
    .venv/Scripts/python.exe scripts/web_target_backup.py      # 9006
    .venv/Scripts/python.exe scripts/web_target_ssrf.py        # 9007

用法：
    VERIFY_IDS=web-006,web-007 VERIFY_PROVIDER=qwen \
...
async def main() -> None:
    ...
    ids = [x.strip() for x in os.getenv("VERIFY_IDS", "web-009").split(",") if x.strip()]
