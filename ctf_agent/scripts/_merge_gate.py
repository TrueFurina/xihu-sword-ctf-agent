...
REGRESSION_CHECKS = [
    ...
    # 2026-09-03 带证据晋级：勒让德符号逐位分解确定性求解器 skills/crypto_legendre_phi.py
    # 接入 presolve（_try_legendre_phi），实测 REGRESS_PASS（phi 分解 1024-bit p/q，sha256 匹配题面真值）。
    {
        "id": "real_crypto_simplelegendre",
        "cmd": [sys.executable, "scripts/_regress_one.py", "real_crypto_simplelegendre"],
        "flag_contains": "REGRESS_PASS",
    },
    # 2026-09-03 带证据晋级：phi+双模逆二次分解确定性求解器 skills/crypto_modinv_factor.py
    # 接入 presolve（_try_modinv_factor），实测 REGRESS_PASS（判别式开方分解 1024-bit p/q，sha256 匹配题面真值）。
    {
