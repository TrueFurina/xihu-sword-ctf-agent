...
PATHS = {
    "RSA Attack": os.path.join(ROOT, "data/questions_real/_attachments/hg2022/RSA Attack"),
    "RSA Attack 2": os.path.join(ROOT, "data/questions_real/_attachments/hg2022/RSA Attack 2"),
    "RSA Attack 3": os.path.join(ROOT, "data/questions_real/_attachments/hg2022/RSA Attack 3"),
}
...
def main() -> int:
    ok_all = True
    # ── 仓库完整性防护: 内部附件目录缺失即优雅退出(不裸崩、退出码 2) ──
    missing = [name for name, path in PATHS.items() if not os.path.isdir(path)]
    if missing:
        print("=== 内部真题附件缺失 (INTERNAL-ATTACHMENT-MISSING) ===")
        for name in missing:
            print(f"  缺失目录: {PATHS[name]}")
        print("=== 说明: 附件应位于仓库 data/questions_real/_attachments/hg2022/ (2026-08-27 将功补过重建);")
        print("===       若目录缺失说明仓库损坏,本题为外部真题(非平台题),不计入严格 KPI。退出码 2。")
        return 2
    for name, path in PATHS.items():
        if not os.path.isdir(path):
            print(f"[X] {name}: 目录不存在 {path}")
            ok_all = False
            continue
        ...
        if name == "RSA Attack 2":
            ...
            try:
                ...
                print(f"[X] {name}: 求解异常 {exc}")
                ok_all = False
                continue
            flag = flag.decode() if isinstance(flag, bytes) else flag

        if not flag:
            print(f"[X] {name}: 未复现解出")
            ok_all = False
            continue
        if flag == expected:
            print(f"[OK] {name}: 复现一致 via={via} ({flag[:24]}...)")
        else:
            print(f"[X] {name}: flag 不一致\n  实测: {flag}\n  台账: {expected}")
            ok_all = False
    print()
