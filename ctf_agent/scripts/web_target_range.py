"""web 练兵场路由靶机（9001）：web-005 路径穿越 / web-008 JWT弱密钥 / web-010 备份泄露 / web-011 SSRF。

仅本地训练用，self_authored_training，不计入任何 KPI。
目的：验证主 Agent 在四类不同 web 漏洞上的路由/工具选择泛化（LLM 贡献 3->6）。
...
- web-011 SSRF：?url= 让服务端代发请求抓取该 URL 并返回 body；内部端点 /web-011/flag 返回 flag
  （toolkit 新增 _FALLBACK_SSRF 对接：下发 url=http://127.0.0.1:9001/web-011/flag 命中）。
...
class _Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ── web-011 SSRF（服务端代发请求）──────────────────
        if route == "/web-011/flag":
            self._send(FLAG)  # 仅服务端本地可直达的内部 flag 端点
            return
        if route == "/web-011":
            if "url" in qs and qs["url"]:
                # 经典 SSRF：服务端代目标去抓取 url 参数指定的资源
                target = qs["url"][0]
                try:
                    req = urllib.request.Request(
                        target, headers={"User-Agent": "range-ssrf-solver"}
                    )
                    ...
                    self._send(f"fetch failed: {e}")
                return
            self._send(
                "<!DOCTYPE html><html><head><title>SSRF</title></head>"
                "<body><h1>SSRF Lab</h1>"
                "<p>hint: ?url= 让服务端代发请求，试试 "
                "url=http://127.0.0.1:9001/web-011/flag</p>"
                "</body></html>"
            )
            return
...
def _standalone() -> None:
    ...
    print(f"  预期 flag: {FLAG}  (routes: /web-005 /web-008 /web-010 /web-011)")
    print("按 Ctrl+C 退出")
    try:
